...
Contains
  ...
  Subroutine halo_dim_plan_fill_3d( plan, which, lbd, ind_range, data, data_with_halo )
    ...
    Integer                    , Dimension( 1:2, 1:3                        ), Intent( In    )              :: ind_range
    ...
    Case( FILL_X )
       ...
       Call plan%left%swap_3d_x ( lbd  , ind_range, data, lbd_h, temp )
       Call plan%right%swap_3d_x( lbd_h, ind_range, temp, lbd_h, data_with_halo )

    Case( FILL_Y )
       lbd_h( 2 ) = lbd_h( 2 ) - plan%left%n_halo
       Allocate( temp( lbd_h( 1 ):ubd_h( 1 ), lbd_h( 2 ):ubd_h( 2 ), lbd_h( 3 ):ubd_h( 3 ) ) )
       Call plan%left%swap_3d_y ( lbd  , ind_range, data, lbd_h, temp )
       Call plan%right%swap_3d_y( lbd_h, ind_range, temp, lbd_h, data_with_halo )

    Case( FILL_Z )
       lbd_h( 3 ) = lbd_h( 3 ) - plan%left%n_halo
       Allocate( temp( lbd_h( 1 ):ubd_h( 1 ), lbd_h( 2 ):ubd_h( 2 ), lbd_h( 3 ):ubd_h( 3 ) ) )
       Call plan%left%swap_3d_z ( lbd  , ind_range, data, lbd_h, temp )
       Call plan%right%swap_3d_z( lbd_h, ind_range, temp, lbd_h, data_with_halo )
  ...
  Subroutine swap_real_3d_x( plan, lbd, ind_range, data, lbd_h, data_with_halo )
    ...
    Select Case( plan%direction )
    Case( LEFT )
       Call plan%swap_left_3d_x ( lbd, ind_range, data, lbd_h, data_with_halo )
    Case( RIGHT )
       Call plan%swap_right_3d_x( lbd, ind_range, data, lbd_h, data_with_halo )
    End Select

  End Subroutine swap_real_3d_x

  Subroutine swap_real_3d_y( plan, lbd, ind_range, data, lbd_h, data_with_halo )
    ...
    Select Case( plan%direction )
    Case( LEFT )
       Call plan%swap_left_3d_y ( lbd, ind_range, data, lbd_h, data_with_halo )
    Case( RIGHT )
       Call plan%swap_right_3d_y( lbd, ind_range, data, lbd_h, data_with_halo )
    End Select

  End Subroutine swap_real_3d_y

  Subroutine swap_real_3d_z( plan, lbd, ind_range, data, lbd_h, data_with_halo )
    ...
    Select Case( plan%direction )
    Case( LEFT )
       Call plan%swap_left_3d_z ( lbd, ind_range, data, lbd_h, data_with_halo )
    Case( RIGHT )
       Call plan%swap_right_3d_z( lbd, ind_range, data, lbd_h, data_with_halo )
    End Select
  ...
  Subroutine swap_real_3d_left_x( plan, lbd, ind_range, data, lbd_h, data_with_halo )
  ...
  Subroutine swap_real_3d_right_x( plan, lbd, ind_range, data, lbd_h, data_with_halo )
  ...
  Subroutine swap_real_3d_left_y( plan, lbd, ind_range, data, lbd_h, data_with_halo )
  ...
  Subroutine swap_real_3d_right_y( plan, lbd, ind_range, data, lbd_h, data_with_halo )
  ...
  Subroutine swap_real_3d_left_z( plan, lbd, ind_range, data, lbd_h, data_with_halo )
    ...
    Do i = 1, Size( plan%steps )
       ...
       If( n_want > 0 ) Then
          ...
          n_msg = nz * n_loc_x * n_loc_y
          If( is_contiguous ) Then
             Call mpi_irecv( data_with_halo( ind_range( 1, 1 ), ind_range( 1, 2 ), got( 1 ) ), n_msg, plan%real_handle, prev, &
                  SWAP_3D_LEFT, comm, requests( 2 ), error )
          Else
             Call mpi_irecv( buffer_recv, n_msg, plan%real_handle, prev, SWAP_3D_LEFT, comm, requests( 2 ), error )
          End If
       Else
  ...
  Subroutine swap_real_3d_right_z( plan, lbd, ind_range, data, lbd_h, data_with_halo )
