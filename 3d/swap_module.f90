...
Contains
  ...
  Subroutine halo_dim_plan_fill_3d( plan, which, lbd, data, data_with_halo )
    ...
    Case( FILL_X )
       Call plan%left%swap_3d_x ( lbd, data, temp           )
  ...
  Subroutine swap_real_3d_x( plan, lbd, data, data_with_halo )
    ...
    Select Case( plan%direction )
    Case( LEFT )
       Call plan%swap_left_3d_x( lbd, data, data_with_halo )
    Case( RIGHT )
       Call plan%swap_right_3d_x( lbd, data, data_with_halo )
    End Select
  ...
  Subroutine swap_real_3d_left_x( plan, lbd, data, data_with_halo )
  ...
  Subroutine swap_real_3d_right_x( plan, lbd, data, data_with_halo )
  ...
  Subroutine swap_real_3d_left_y( plan, lbd, data, data_with_halo )
    ...
    Do i = 1, Size( plan%steps )
       ...
       ! Send out data and Recieve new data
       If( n_wanted > 0 ) Then
          Call copy_in( [ 1, n_loc_x ], can_give, [ 1, n_loc_z ], Lbound( data_with_halo ), data_with_halo, buffer_send )
       ...
       If( n_want > 0 ) Then
          ny = got( 2 ) - got( 1 ) + 1
          n_msg = ny * n_loc_x * n_loc_z
          Call copy_out( [ 1, n_loc_x ], got, [ 1, n_loc_z ], Lbound( data_with_halo ), buffer_recv, data_with_halo )
       End If
